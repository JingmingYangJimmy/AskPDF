...
console.log("REACT_APP_DOMAIN =", DOMAIN); // ← add this
...
const ChatComponent = (props) => {
  ...
  //user does not need to click search button, after listening, search immediately
  useEffect(() => {
    if (!listening && !!transcript) {
      //IIFE - immediately invoked function expression, let onSearch finished, and then do next
  ...
  const onSearch = async (question) => {
    ...
    try {
      ...
      console.log(error);
      handleResp(question, "something went wrong, try again");
    } finally {
      setIsLoading(false);
    }
  };
